  ...
  return (
    <html lang="es">
      <body className="min-h-screen bg-slate-50 antialiased">
            ...
            <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="flex items-center justify-between h-16">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h1 className="text-xl font-bold bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent">Assistravel</h1>
                    <p className="text-xs text-slate-500 font-medium">Gestión de Casos</p>
                  </div>
                </div>
                <nav className="flex items-center gap-1.5">
                  <Link
                    href="/"
                    className="flex items-center gap-2 px-3.5 py-2 text-sm font-medium text-slate-600 hover:text-blue-600 hover:bg-blue-50/70 rounded-lg transition-all duration-200"
                  >
        ...
        <main className={`max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 ${user ? "py-8" : ""}`}>
          {children}
        </main>
        {user && (
          <footer className="border-t border-slate-200 bg-white mt-auto">
            <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
              <p className="text-center text-sm text-slate-400 font-medium">
                © {new Date().getFullYear()} <span className="text-slate-600">Assistravel</span> · Sistema de Gestión de Casos
              </p>
            </div>
          </footer>
        )}
